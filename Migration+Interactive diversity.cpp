...
int main(void) {
    int Round = 10000;          // Total rounds per experiment
        ...
        for (r = 3.0; r <= 5.00; r += 0.01) {
            ...
            for (int exp = 0; exp < Experiments; exp++) {
                ...
                for (int i = 0; i < Round; i++) {
                    ...
                    // Collect data only for the last 2000 rounds
                    if (i >= Round - 2000) {
                        for (int j = 0; j < 3; j++) {
                ...
                for (int j = 0; j < 3; j++) {
                    double avg_strategy = data_sum[j] / 2000;
                    total_data[j] += avg_strategy;
                    exp_results[j].push_back(avg_strategy);

                    double avg_payoff = data_sum_payoff[j] / 2000;
