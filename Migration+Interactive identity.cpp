...
int main(void) {
    int Round = 1000000;     // Total number of rounds per experiment
    ...
    for (double rho = 0.10; rho <= 1.00; rho += 0.01) {
        ...
        for (r = 3.0; r <= 5.00; r += 0.01) {
            ...
            for (int exp = 0; exp < Experiments; exp++) {
                ...
                for (int j = 0; j < 3; j++) {
                    double avg_strategy = data_sum[j] / 5000; // Compute mean cooperation fraction
                    total_data[j] += avg_strategy;  // Accumulate total sum
                    exp_results[j].push_back(avg_strategy); // Store experiment results

                    double avg_payoff = data_sum_payoff[j] / 5000; // Compute mean payoff
                ...
                printf("rho = %.2f, r = %.2f, Experiment = %d: Fc_m = %.4f, Fc_nm = %.4f, Fc = %.4f, "
                       ...
                       rho, r, exp + 1, data_sum[0] / 5000, data_sum[1] / 5000, data_sum[2] / 5000,
                       data_sum_payoff[0] / 5000, data_sum_payoff[1] / 5000, data_sum_payoff[2] / 5000);
            }
