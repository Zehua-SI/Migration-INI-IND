...
#include <cstdlib>      // Standard library functions like `rand()`, `srand()`, etc.
#include <cstdio>       // Standard C I/O library
#include <cmath>        // Math functions like `exp()`, `sqrt()`, etc.
#include <ctime>        // Time library for initializing random seed
...
double strategy[N];          // Strategy value of each player (0 = Defector, 1 = Cooperator)
...
void init_game(double rho) {
    ...
    for (int i = 0; i < N; i++) {
        if (randf() < rho) { // A site is occupied with probability `rho`
            ...
            // Set strategy value randomly to 0 (defector) or 1 (cooperator) with equal probability
...
bool can_move(int x) {
    ...
    int cooperators_neighbors = 0;    // Count of cooperating neighbors
    ...
    for (int i = 0; i < neig_num; i++) {
        ...
        if (strategy[neig] != -1.0) {  // If the neighbor is occupied
            ...
            if (strategy[neig] == 1.0) {
                cooperators_neighbors++;  // Count cooperating neighbors
            }
        }
    }

    // Calculate the ratio of cooperators among neighbors
    double coop_ratio = (occupied_neighbors > 0) ?
                        ((double)cooperators_neighbors / occupied_neighbors) : 0.0;

    // Move if all neighbors are empty or if the cooperator ratio is too low
...
double cal_payoff(int x) {
    ...
    if (group_members.size() > 1) {
        ...
        // Count cooperators in the group
        cooperators += (strategy[x] == 1.0) ? 1 : 0;
        for (int player : group_members) {
            if (player == x) continue; // Skip self
            cooperators += (strategy[player] == 1.0) ? 1 : 0;
        }

        // Compute benefits and cost
        double total_benefit = r * cooperators;
        double payoff_i = total_benefit / group_members.size();
        if (strategy[x] == 1.0) { // If cooperator, subtract cooperation cost
            payoff_i -= c;
        }
...
void round_game() {
    ...
    for (int i : current_occupied) {
        if (strategy[i] == 0.0 || strategy[i] == 1.0) { // Ensure site is still occupied
            learn_strategy(i);
        }
    }
}
...
// Compute statistical data for cooperation rates and payoffs
void cal_data() {
...
int main(void) {
    int Round = 10000;     // Total number of rounds per experiment
    int Experiments = 50;   // Number of independent experiments

    sgenrand(time(0));       // Initialize the random seed
    ...
    // Loop over player density (rho) from 0.10 to 1.00 in increments of 0.01
    for (double rho = 0.10; rho <= 1.00; rho += 0.01) {
        // Loop over public goods synergy factor (r) from 3.0 to 5.0 in increments of 0.01
        for (r = 3.0; r <= 5.00; r += 0.01) {
            ...
            for (int exp = 0; exp < Experiments; exp++) {
                ...
                double data_sum[3] = { 0.0, 0.0, 0.0 }; // Sum of cooperation fractions over last steps
                double data_sum_payoff[3] = { 0.0, 0.0, 0.0 }; // Sum of payoffs over last steps
                ...
                // Compute average cooperation fraction and average payoff over last 2000 rounds
                for (int j = 0; j < 3; j++) {
                    double avg_strategy = data_sum[j] / 2000; // Compute mean cooperation fraction
                    total_data[j] += avg_strategy;  // Accumulate total sum
                    exp_results[j].push_back(avg_strategy); // Store experiment results

                    double avg_payoff = data_sum_payoff[j] / 2000; // Compute mean payoff
                    total_payoff_data[j] += avg_payoff; // Accumulate total sum
                    exp_payoff_results[j].push_back(avg_payoff); // Store experiment results
                }

                // Print progress to the console
                printf("rho = %.2f, r = %.2f, Experiment = %d: Fc_m = %.4f, Fc_nm = %.4f, Fc = %.4f, "
                       "Payoff_m = %.4f, Payoff_nm = %.4f, Payoff = %.4f\n",
                       rho, r, exp + 1, data_sum[0] / 2000, data_sum[1] / 2000, data_sum[2] / 2000,
                       data_sum_payoff[0] / 2000, data_sum_payoff[1] / 2000, data_sum_payoff[2] / 2000);
            }
            ...
            for (int j = 0; j < 3; j++) {
                mean[j] = total_data[j] / Experiments; // Compute mean cooperation fraction
                calculate_standard_deviation(exp_results[j], mean[j], std_dev[j]); // Compute standard deviation

                mean_payoff[j] = total_payoff_data[j] / Experiments; // Compute mean payoff
                calculate_standard_deviation(exp_payoff_results[j], mean_payoff[j], std_dev_payoff[j]); // Compute standard deviation
            }
